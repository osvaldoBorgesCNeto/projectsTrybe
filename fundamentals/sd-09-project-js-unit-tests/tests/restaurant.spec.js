...
// Creado a funcao orderFunc

const orderFunc = (string) => {
  objetoRetornado.consumption.push(string);
}

// Creado a funcao payFunc

const payFunc = (array) => {
  let sum = 0;
  for (let index = 0; index < array.length; index += 1) {
    for (let index2 = 0; index <= Object.keys(objetoRetornado.fetchMenu.food).length; index += 1) {
      if (array[index] === Object.keys(objetoRetornado.fetchMenu.food)[index2]) {
        sum += Object.values(objetoRetornado.fetchMenu.food)[index2];
      }
    }
  }
  for (let index = 0; index < array.length; index += 1) {
    for (let index2 = 0; index <= Object.keys(objetoRetornado.fetchMenu.drink).length; index += 1) {
      if (array[index] === Object.keys(objetoRetornado.fetchMenu.drink)[index2]) {
        sum += Object.values(objetoRetornado.fetchMenu.drink)[index2];
      }
    }
  }
  return sum;
...
const consumptionVar = [];

let objetoRetornado = createMenu(myOject);
objetoRetornado.consumption = consumptionVar;
objetoRetornado.order = orderFunc;
...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    // ```
    assert.deepStrictEqual(createMenu(myOject), { fetchMenu: myOject});
    ...
    // ```
    assert.deepStrictEqual(myOject, objetoRetornado.fetchMenu);
    ...
    // ```
    const result = objetoRetornado.pay(objetoRetornado.consumption);
    assert.strictEqual(result, 11.7);
